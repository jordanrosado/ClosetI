...
mali_update() {
	if [ "$BOARD" != "odroidxu" ]; then
		...
		if [ "$DISTRO" = "debian" ]; then
			...
			apt-get -y install build-essential git xz-utils xserver-xorg-dev libudev-dev 
		fi
...
install_zImage_lcdkit() {
	cp $TEMP/boot/zImage.lcdkit /media/boot
	sync
}
